...
function show2doFile(args, callback) {
    var arrToReturnAndTest = []; // this is created only for tests
    var f = new Promise((res, rej) => { // we are useing promises so that we can test our code
...
function todoMain() {
    ...
    setTimeout(() => {
        ...
        vorpal.command('version', 'Logs current version. ')
            .action(function (args, callback) {
                console.log("Current version is : " + chalk.yellow(package.version));
                callback();
            });
...
module.exports = {
    todoMain: todoMain,
    write2doFile: write2doFile,
    show2doFile: show2doFile,
    list2do: list2do,
    add2do: add2do,
    delete2do: delete2do
}