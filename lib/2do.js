...
function todoMain() {
    ...
    setTimeout(function () {
         ...
         * init command
         */
        vorpal.command('init', 'Outputs "bar".')
            .action(function (args, callback) {
                ...
                fs.stat('2do.json', function (err, stats) {
                    ...
                    if (stats) {
                        console.log(chalk.red('You already have a 2do in this repository.'))
                    } else {
                        fs.writeFile('2do.json', "{}", (err) => {
                            if (err) throw err;
                            console.log(chalk.green('Ok project started successfuly. '));
                        });
                    }
                });
                callback();
         ...
         * --help command
         */
        vorpal.command('--help', 'Outputs "bar".')
            .action(function (args, callback) {
                console.log(`
                  Help
         ...
         * show command
         */
        vorpal.command('show [fileName]', 'Outputs "bar".')
            .action(function (args, callback) {
         ...
         * list command
         */
        vorpal.command('list', 'Outputs "bar".')
            .action(function (args, callback) {
                fs.readFile('2do.json', (err, data) => {
                    if (err) throw err;
                    var stringOfJSON = data.toString();
         ...
         * add command
         */
        vorpal.command('add <fileName> [title]', 'Outputs "bar".')
            .action(function (args, callback) {
         ...
         * delete command
         */
        vorpal.command('delete <fileName> [todoNumber]', 'Outputs "bar".')
            .action(function (args, callback) {
                fs.readFile('2do.json', (err, data) => {
                    if (err) throw err;
                    var stringOfJSON = data.toString();
